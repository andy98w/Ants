...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            #check to see if ant is container ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        oogabooga = self.place
        distance = 0
        while oogabooga.is_hive != True:
            if self.lower_bound <= distance <= self.upper_bound and len(oogabooga.bees) > 0:
                return random_bee(oogabooga.bees)
            oogabooga = oogabooga.entrance
            distance += 1
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        randomb = random_bee(bee)
        if randomb != None:
            randomb.reduce_health(randomb.health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        if (self.ant_contained is None) and (not other.is_container) and (self.is_container is True):
            return True
        return False
        # END Problem 8a
